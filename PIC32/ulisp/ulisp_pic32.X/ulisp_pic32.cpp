/* 
 * File:   ulisp_pic32.cpp
 * Author: micha
 * 
 * Created on 9. Januar 2020, 21:43
 */
...
// Bemerkung: baudrate wirt via Tool MCC gesetzt !        
        UART1_Initialize();
    }
...
#define CODE_FOR_ZERO
#undef CODE_FOR_ESP32

#ifdef CODE_FOR_ESP32
...
#define boolean bool
#define PROGMEM
#define STR_PTR const char * const *
...
// TODO --> check !!!
int pgm_read_word(const char * const * ptr)
{
    int * p = (int *)ptr;
    return *p;
}

char pgm_read_byte(int ptr)
{
    char * p = (char *)ptr;
    return *p;
}
...
int builtin (char* n) {
  ...
  while (entry < ENDFUNCTIONS) {
   if (strcmp/*_P*/(n, (PGM_P)pgm_read_word(&lookup_table[entry].string)) == 0 )
      return entry;
    entry++;
  }
  return ENDFUNCTIONS;
...
fn_ptr_type lookupfn (symbol_t name) {
  return (fn_ptr_type)pgm_read_word((STR_PTR)&lookup_table[name].fptr);
}

int lookupmin (symbol_t name) {
  return pgm_read_word((STR_PTR)&lookup_table[name].min);
}

int lookupmax (symbol_t name) {
  return pgm_read_word((STR_PTR)&lookup_table[name].max);
}

char *lookupbuiltin (symbol_t name) {
  strcpy/*_P*/(Buffer, (PGM_P)(pgm_read_word(&lookup_table[name].string)));
  return Buffer;
}
